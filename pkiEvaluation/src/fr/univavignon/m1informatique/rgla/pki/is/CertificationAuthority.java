...
public class CertificationAuthority extends ISComponent implements ICertificationAuthority {
	protected Certificate certificate;
	protected DistinguishedName hashName;
	protected Period initialPeriod;
	protected AsymetricKey authorityKey;
	private long serialNumber;
	protected long serialNumberReference;
	protected List<Certificate> certificateRevocationList;

	public CertificationAuthority(DistinguishedName unitName, DistinguishedName hashName, Period initialPeriod)
	...
	public boolean isCertificateValid(Certificate certificate) throws PKIException {
		...
		return isCertificateValid(certificate.reduceChain());
	...
	public Certificate signCertificate(CertificateSigningRequest certificateSigningRequest) throws PKIException {
		CertificateSigningRequestData certificateSigningRequestData = certificateSigningRequest.getObject();
		CertificateElementData certificateElementData = new CertificateElementData(serialNumberReference++,
				certificateSigningRequestData);
		Certificate certificateOfSon = new Certificate(certificate);
		try {
			CertificateElement certificateElement = new CertificateElement(certificateElementData, getPrivateKey(),
					hashName);
	...
	public void revokeCertificate(CertificateRevocationRequest certificateRevocationRequest) throws PKIException {
		certificateRevocationList.add(certificateRevocationRequest.getCertificate());
	}
	...
	public void createCertificate(Period period, CertificateRight certificateRight, DistinguishedName hashName)
		...
		CertificateSigningRequestData certificateSigningRequestData = new CertificateSigningRequestData(this.getDN(),
				authorityKey.getPublicKey(), hashName, period, certificateRight);
		try {
			...
			if (certificate == null) {
				certificate = new Certificate(signCertificate(certificateSigningRequest));
			} else {
				CertificationAuthority myLastAuthority = (CertificationAuthority) certificate
						.getLastAuthorityCertificateElement().getSubjectName().getObject();
				certificate = myLastAuthority.signCertificate(certificateSigningRequest);
			}
	...
	@Override
	public Certificate renewCertificate(CertificateRenewalRequest certificateRenewalRequest) throws PKIException {
		Certificate oldCertificate = certificateRenewalRequest.getCertificate();
		CertificateElement oldCertificateElement = oldCertificate.getLastCertificateElement();
		Period newCertificatePeriod = new Period(System.currentTimeMillis(), certificateRenewalRequest.getExpire());
		CertificateSigningRequestData certificateSigningRequestData = new CertificateSigningRequestData(
				oldCertificateElement.getSubjectName(), oldCertificateElement.getPublicKey(),
				oldCertificateElement.getHashName(), newCertificatePeriod, oldCertificateElement.getCertificateRight());
		CertificateElementData renewCertificateElementData = new CertificateElementData(
				oldCertificateElement.getSerialNumber(), certificateSigningRequestData);
		try {
			CertificateElement renewCertificateElement = new CertificateElement(renewCertificateElementData,
					getPrivateKey(), hashName);
